...
export default function ApprovalsScreen({showmonthDependency}: any) {
    // GLOBAL STATES ------------------------------------>
    ...
    const fetchQuery = async () => {
        ...
        const url = `https://illimitable.in/app/mobile/late-attendance-for-supervisor.json`;
    ...
    const {isLoading, error, data} = useQuery({
        queryKey: ['lateApprovals', showmonthDependency.exactDate],
        queryFn: fetchQuery,
        enabled: !!showmonthDependency,
    });

    // Triggers refetch every time the screen is focused
    useRefetchOnFocus(['lateApprovals']);

    // RENDER ITEM FLATLIST ------------------------------------------->
    const renderItem = ({item}: {item: ApprovalDataType}) => {
        console.log(item);

        return <LateCard approveButton={true} item={item} />;
    };
